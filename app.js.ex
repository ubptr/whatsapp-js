...
async function addParticipantsToGroup(groupId, participants) {
    try {
        ...
        // Ensure that participants are in the correct format
...
app.get('/send-media', async (req, res) => {
    ...
    try {
    //     const { data: mediaData } = await axios.get(mediaUrl, {responseType: "arraybuffer"})
	// const media = new MessageMedia('image/png', mediaData.toString("base64")
        // );
...
app.get('/add-member', async (req, res) => { 
    const { groupId, number, caption = '' } = req.query; // Use query parameters

    try {
        await addParticipantsToGroup(groupId, number);
        res.send({ success: true, message: "Add Successfully" });
        console.log('Participants added successfully.');
    } catch (err) {
        console.error('Failed to add participants:', err);
        res.status(500).send({ success: false, message: "Failed to add.", error: err.toString() });
    }
});